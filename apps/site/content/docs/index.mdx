---
title: ObjectQL
description: The Universal Data Compiler - Stop writing glue code. Start compiling your data layer.
---

# ObjectQL

## The Universal Data Compiler

Stop writing glue code. Start compiling your data layer. Define schemas once in YAML, compile to optimized SQL for any database, auto-generate type-safe APIs.

## Features

### 🔒 Zero-Overhead Security

RBAC and ACL rules are compiled directly into SQL WHERE clauses at query time, not checked in application memory. Security breaches from forgotten permission checks are impossible.

### 🌐 Database Agnostic

Write your schema once. Switch from local SQLite in development to production PostgreSQL, MySQL, or TiDB without changing a single line of code. The compiler handles dialect translation.

### ⚡ Virtual Columns

Define computed fields in your schema that compile to efficient database expressions. No N+1 queries, no manual JOIN management—just declare the intent and let the compiler optimize.
