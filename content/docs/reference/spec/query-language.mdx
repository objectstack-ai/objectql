---
title: "ObjectQL Query Protocol (JSON-DSL)"
description: ObjectQL Query Protocol (JSON-DSL)
---
**Version:** 1.0.0
...
```typescript
// Modern filter condition type (object-based syntax)
// Supports MongoDB/Prisma-style operators: $eq, $gt, $gte, $lt, $lte, $ne, $in, $nin, etc.
// For full operator reference, see Section 3.2
export type FilterCondition = {
  [field: string]: 
    | any  // Implicit equality: { status: "active" }
    | { [operator: string]: any }  // Explicit operator: { price: { $gt: 100 } }
} | {
...
export interface UnifiedQuery {
  ...
  // === 3. Selection (WHERE) ===
  // Modern object-based filter syntax (MongoDB/Prisma-style)
  filters?: FilterCondition; // See Section 3 for detailed syntax
...
## 3. Modern Filter Syntax

ObjectQL uses a **modern object-based filter syntax** inspired by MongoDB and Prisma. This replaces the legacy array-based syntax and provides better type safety, readability, and IDE support.

> **For detailed filter examples and patterns**, see the [Filter Syntax Guide](../../data-access/filter-syntax.mdx).
...
Multiple fields at the top level are implicitly combined with AND logic.

#### Explicit Operators

Use operators for more complex conditions:
...
```json
{
  "object": "users",
  "filters": {
    "deletedAt": { "$null": true },     // IS NULL (record is not deleted)
    "lastLoginAt": { "$null": false }   // IS NOT NULL (user has logged in)
  }
}
```
...
Top-level fields are implicitly AND-ed, but you can use `$and` explicitly:
...
**SQL Equivalent:** `WHERE (category = 'Electronics' OR category = 'Computers') AND status = 'active' AND price > 100`

### 3.6 Legacy Array-Based Syntax (Deprecated)

> **⚠️ Deprecated:** The array-based filter syntax is still supported for backward compatibility but is **not recommended** for new code. Use the modern object-based syntax instead.

The legacy syntax used array tuples:
...
For migration guidance, see the [Filter Syntax Guide](../../data-access/filter-syntax.mdx).
...
```javascript
{
  ...
  // Modern filter syntax: (Paid OR Pending) AND Amount > 100 AND Recent
  "filters": {
    "$and": [
      {
        "$or": [
...
```javascript
{
  ...
  "filters": {
    "$and": [
      ...
      {
        "$or": [
          { "end_date": { "$lt": "$today" } },  // Overdue (special variable)
          { "budget_exceeded": true },  // Over budget flag
          { "risk_level": "high" }  // High risk flag
        ]
      }
    ]
  },
...
"filters": {
  "$and": [
    // Your original filters (if any) would go here
    {
      "$or": [
        { "created_at": { "$lt": "2024-01-14" } },
        {
          "$and": [
