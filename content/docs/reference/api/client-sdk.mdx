---
title: "Client SDK Usage Guide"
description: Client SDK Usage Guide
---
...
interface User {
  id: string;  // Note: Use 'id' not '_id' (unified field name across all databases)
  name: string;
  email: string;
...
**Note:** The unified `id` field is used across all database drivers (MongoDB, SQL, etc.). For MongoDB migration from `_id`, see the [Migrations Guide](../../modeling/migrations.mdx).
...
console.log(newUser.id); // Generated ID (unified field name)
...
- [REST API Documentation](/docs/reference/api/rest)
- [Metadata API Documentation](/docs/reference/api/metadata)
- [Error Handling Guide](/docs/reference/api/error-handling)