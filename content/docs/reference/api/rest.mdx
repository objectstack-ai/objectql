...
The REST API accepts query parameters that map to the [Query Language](../spec/query-language.mdx) specification:

- `filter` - JSON-encoded filter array (optional)
- `sort` - Field name or comma-separated list (optional)
- `top` - Maximum number of records to return (LIMIT, recommended: 10-100)
...
```bash
GET /api/data/users?filter=[["status","=","active"]]&sort=created_at&top=20&skip=0
```
