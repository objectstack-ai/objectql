...
description: Security best practices and permission implementation patterns
---

## Security Model

ObjectQL provides a **framework for security**, but **requires application-layer implementation** of permission checks and access control.

> **⚠️ Important**: The core ObjectQL engine does NOT automatically enforce permissions. You must implement security checks in your application layer using hooks, middleware, or custom logic.
...
| **Authorization/RBAC** | ⚠️ Application Layer | Implement using hooks |
| **Row-Level Security** | ⚠️ Application Layer | Implement in `beforeFind` hooks |
| **Field-Level Security** | ⚠️ Application Layer | Implement in hooks or use GraphQL resolvers |
| **Audit Logging** | ⚠️ Application Layer | Implement using `after*` hooks |
...
The ObjectQL team is planning native support for:

- **Built-in RBAC engine** - Declarative permission rules
- **Permission metadata** - Define permissions in `.permission.yml` files
- **Automatic enforcement** - Framework-level permission checks
- **Role inheritance** - Complex role hierarchies

> **📋 Track progress**: See [Implementation Status](https://github.com/objectstack-ai/objectql/blob/main/IMPLEMENTATION_STATUS.md) for current development status.
